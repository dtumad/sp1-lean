...
[`axiom-census.txt`](axiom-census.txt) (the main `SP1Clean` library, 2511 declarations) and
...
- 2813 released declarations are probed.
...
The CPU Memory-accounting integration adds fifteen main declarations: three use the logical
baseline, three retain the existing 77-dependency execution-view set, and nine retain the installed
...
installed source registry derives its zero gate. This result assumes neither successful replay
nor prior Memory guarantees. Combined-touch alignment, refresh elimination, predecessor value
truth, and complete host execution grounding remain open. The preceding grouping regression's
