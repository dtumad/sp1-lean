...
/-- Compose an execute stage observing the actual incoming registers and staged successor with
official retirement. Instruction families discharge the premise from their semantic operands. -/
theorem retire_frame_of_observed_execute {program : GuestProgram} {source : SailState}
  ...
  have stagedNext : staged.regs.get? Register.nextPC = some (pc + 4#64) := by
    ...
    rfl
  obtain ⟨post, executed, postCfg, memory⟩ := executeFrame staged stagedCfg observed stagedPc stagedNext
...
/-- Recover the actual direct execute result from normal retirement and transport its frame
through official Sail bookkeeping. Instruction families discharge the execute premise. -/
theorem normal_frame_of_direct_execute {program : GuestProgram} {source target : SailState}
  ...
  obtain ⟨direct, afterCfg, memory⟩ := frame staged stagedCfg result afterExecute executeRun
  ...
  cases targetRun
  refine ⟨?_, memoryTail.trans memory⟩
