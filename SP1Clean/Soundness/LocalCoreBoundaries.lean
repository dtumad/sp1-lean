...
def sourceWitness {image : ProgramImage} {snapshot : MemorySnapshot}
    (witness : EnsembleWitness (ensemble (p := p) image snapshot)) :
    EnsembleWitness ((SnapshotMemoryEnsemble.inventory (p := p) snapshot).ensemble
...
theorem sourceTables_spec {image : ProgramImage} {snapshot : MemorySnapshot}
    (witness : EnsembleWitness (ensemble (p := p) image snapshot))
    (constraints : witness.Constraints) (balanced : witness.BalancedChannels) :
    ∀ table ∈ (sourceWitness witness).tables.take (SnapshotMemoryEnsemble.inventory (p := p) snapshot).views.length,
      table.Spec := by
  intro table member row rowMem
  have componentMem : table.component ∈ (SnapshotMemoryEnsemble.inventory (p := p) snapshot).views.map (·.component) := by
  ...
  have byte := (finishedChannel_guarantees image snapshot witness constraints balanced table tableMem).1 row rowMem
  have checked := constraints table tableMem row rowMem
  rw [← same] at byte checked ⊢
  obtain ⟨id, _, rfl⟩ := List.mem_map.mp viewMem
  exact SnapshotMemoryEnsemble.view_spec snapshot id _ checked byte

/-- Every physical source record authenticates its complete value against the fixed snapshot. -/
theorem source_records_authentic {image : ProgramImage} {snapshot : MemorySnapshot}
    (witness : EnsembleWitness (ensemble (p := p) image snapshot))
    (constraints : witness.Constraints) (balanced : witness.BalancedChannels) :
    ∀ record ∈ (SnapshotMemoryEnsemble.inventory snapshot).records (sourceWitness witness),
      MemoryBoundary.SnapshotSpec snapshot record :=
  (SnapshotMemoryEnsemble.inventory snapshot).records_valid_of_tables (sourceWitness witness)
...
theorem verifier_source_interactions (image : ProgramImage) (snapshot : MemorySnapshot) (env : Environment (ZMod p)) :
    (⟨verifier image snapshot⟩ : Component (ZMod p)).operations.interactionValuesWith
...
theorem sourceWitness_interactions {image : ProgramImage} {snapshot : MemorySnapshot}
    (witness : EnsembleWitness (ensemble (p := p) image snapshot)) :
  ...
  change ((SnapshotMemoryEnsemble.inventory (p := p) snapshot).ensemble
...
theorem source_records_locations_nodup {image : ProgramImage} {snapshot : MemorySnapshot}
    (witness : EnsembleWitness (ensemble (p := p) image snapshot))
    (constraints : witness.Constraints) (balanced : witness.BalancedChannels) :
    (((SnapshotMemoryEnsemble.inventory snapshot).records (sourceWitness witness)).map MemoryMsg.locOf).Nodup := by
  apply (SnapshotMemoryEnsemble.inventory snapshot).records_locations_nodup_of_tables
...
theorem source_memory_interactions {image : ProgramImage} {snapshot : MemorySnapshot}
    (witness : EnsembleWitness (ensemble (p := p) image snapshot)) :
    (witness.tables.take (SnapshotMemoryEnsemble.inventory (p := p) snapshot).views.length).flatMap
        (·.interactionsWith memoryChannel.toRaw) =
      ((SnapshotMemoryEnsemble.inventory snapshot).records (sourceWitness witness)).map memoryChannel.pushedValue :=
  (SnapshotMemoryEnsemble.inventory snapshot).memory_interactions_eq (sourceWitness witness)
    memoryChannel.pushedValue (SnapshotMemoryEnsemble.recordFor_interactions snapshot)

/-- Arbitrary public endpoints are canonical and source data passes the finite program, decode,
x0, and ROM checks. No loader or source-ROM-consistency premise is supplied by the caller. -/
theorem public_boundary {image : ProgramImage} {snapshot : MemorySnapshot}
    (witness : EnsembleWitness (ensemble (p := p) image snapshot))
    (constraints : witness.Constraints) (balanced : witness.BalancedChannels) :
    witness.publicInput.LimbBounds ∧ SourceValid image snapshot := by
  have spec : witness.verifierTable.Spec := by
    intro row member
    exact NativeCore.component_spec_of_byte (⟨verifier image snapshot⟩ : Component (ZMod p)) (List.Subset.refl _) _ (by trivial)
      (constraints _ witness.mem_allTables_verifierTable row member)
      ((finishedChannel_guarantees image snapshot witness constraints balanced _
