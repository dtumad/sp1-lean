...
runs the queue and both bank boundaries exactly once. A derived singleton representation preserves all constraints
and the full interaction ledger, so existing installed execution proofs continue to apply.
The final cursor and outgoing bank words are instance parameters. The other fields of `bankFinal`
are unused; complete outgoing snapshot agreement and dynamic allocation history are separate proofs.
-/
...
theorem expanded_allTables (witness : EnsembleWitness (ensemble image source final bankFinal others resources channels)) :
    (expanded witness).allTables = ((boundary source final bankFinal).expand witness).allTables := rfl
...
theorem expanded_constraints (witness : EnsembleWitness (ensemble image source final bankFinal others resources channels))
  ...
  exact (boundary source final bankFinal).expand_constraints witness constraints
...
theorem expanded_interactions (witness : EnsembleWitness (ensemble image source final bankFinal others resources channels))
  ...
  (boundary source final bankFinal).expand_interactions witness channel
...
theorem source_binding (witness : EnsembleWitness (ensemble image source final bankFinal others resources channels))
  ...
  exact SP1Clean.HostBoundary.source_bound _ _ _ _ _ _ raw

omit [Fact (2 ^ 25 < p)] in
/-- The boundary uses only queue and bank state, with no record or CPU/word-cursor edge. -/
theorem boundary_silent (channel : RawChannel (ZMod p)) (different : channel ≠ stateChannel.toRaw) :
  ...
  intro commit deferred
  simp only [boundary, HostBoundary.closed, HostBoundary.circuit, circuit_norm,
    List.mem_cons, List.not_mem_nil, different, commit, deferred, or_self, not_false_eq_true]
...
theorem extraTables_eq
  ...
  have bound : 83 ≤ witness.tables.length := by
    ...
    omega
  change (((witness.tables ++ [(boundary source final bankFinal).singleton witness.data]).drop 60).drop 21).drop 2 = _
